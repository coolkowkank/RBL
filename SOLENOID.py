...
def B_total(N, no_lilitan) : # Nilai B pada no_lilitan dengan konfigurasi N
    y = 0
    ...
    for i in range(len(N)) : # pada layer i+1
        for j in range(N[i]//2) : # pada lilitan ke-j
            jarak_lilitan_ke_pusat = jarak_pusat(N, j, i)
            #untuk sama sisi bagian dari pusat
            z = jarak_dicari - jarak_lilitan_ke_pusat
            z_sebelah = jarak_dicari + jarak_pusat
            y += B(N,z, i)
            y += B(N,z_sebelah, i)
